...
while running:
    ...
    if player == 'O' and not gameover:
        ai_cell = ai_move(cells)
        if ai_cell:
            # Simulate AI placing a line
            ai_action = random.choice([0, 1, 2, 3])
            if not ai_cell.sides[ai_action]:
                ai_cell.sides[ai_action] = True
                index = ai_cell.index
                if ai_action == 0 and index - ROWS >= 0:
                    cells[index - ROWS].sides[2] = True
                elif ai_action == 1 and (index + 1) % COLS > 0:
                    cells[index + 1].sides[3] = True
                elif ai_action == 2 and index + ROWS < len(cells):
                    cells[index + ROWS].sides[0] = True
                elif ai_action == 3 and (index % COLS) > 0:
                    cells[index - 1].sides[1] = True
                res = ai_cell.checkwin(player)
                if res:
                    fillcount += res
                    if player == 'X':
                        p1_score += 1
